...
func (r *RaftServer) Set(command string, conn net.Conn) {
	...
	if err != nil {
		r.Output_ch <- String_Conn{"ERR_CMD_ERR\r\n", conn}
		return
	}
	...
	if len(inputs) == 5 && inputs[4] == "noreply" {
		...
		r.Output_ch <- String_Conn{"OK " + strconv.FormatInt(r.KVStore[key].Version,10) + "\r\n", conn}
		return
	}
}

// Executes the get command
func (r *RaftServer) Get(command string, conn net.Conn) {
	inputs_ := strings.Split(command, "\r\n")
	inputs := strings.Split(inputs_[0], " ")
	key := inputs[1]
	t, present := r.KVStore[key]
	if !present || (present && !t.IsExpTimeInf && t.ExpiryTime.Before(time.Now())) {
		if present { delete(r.KVStore, key) }
		r.Output_ch <- String_Conn{"ERR_NOT_FOUND\r\n", conn}
		return
	} else {
		r.Output_ch <- String_Conn{"VALUE " + strconv.Itoa(t.NumBytes) + "\r\n" + t.Text + "\r\n", conn}
		return
	}
}

// Executes the getm command
func (r *RaftServer) Getm(command string, conn net.Conn) {
	inputs_ := strings.Split(command, "\r\n")
	inputs := strings.Split(inputs_[0], " ")
	key := inputs[1]
	t, present := r.KVStore[key]
	...
	if !present || (present && !t.IsExpTimeInf && t.ExpiryTime.Before(time.Now())){
		...
		r.Output_ch <- String_Conn{"VALUE " + strconv.Itoa(int(t.Version)) + " " + secondsLeft + " " + strconv.Itoa(t.NumBytes) + "\r\n" + t.Text + "\r\n", conn}
		return
	}
}

// Executes the cas command
func (r *RaftServer) Cas(command string, conn net.Conn) {
	inputs_ := strings.Split(command, "\r\n")
	inputs := strings.Split(inputs_[0], " ")
	key := inputs[1]
	seconds, err := strconv.Atoi(inputs[2])
	...
	if len(inputs) == 6 && inputs[5] == "noreply" {
		...
		if t.Version != version {
			r.Output_ch <- String_Conn{"ERR_VERSION\r\n", conn}
			return
		} else {
			r.Output_ch <- String_Conn{"OK " + strconv.FormatInt(r.KVStore[key].Version,10) + "\r\n", conn}
			return
...
func (r *RaftServer) Delete(command string, conn net.Conn) {
	inputs_ := strings.Split(command, "\r\n")
	inputs := strings.Split(inputs_[0], " ")
	key := inputs[1]
	t, present := r.KVStore[key]
	if !present || (present && !t.IsExpTimeInf && t.ExpiryTime.Before(time.Now())){
		...
		delete(r.KVStore, key)
		r.Output_ch <- String_Conn{"DELETED\r\n", conn}
		return
	}
}

// Reads send-requests from the output channel and processes it
func (r *RaftServer) DataWriter() {
	for {
		replych := <-r.Output_ch
		text := replych.Text
...
func (r *RaftServer) Evaluator() {
	for {
		inputch := <-r.Input_ch			// Only one control flow is allowed after this point
...
func (r RaftServer) GetCommand(input string) (string, string) {
